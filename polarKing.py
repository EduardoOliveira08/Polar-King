...
climbDone = False #Indica se a escalada (climbing) foi concluída
...
throwBarril = False # Indica se o urso está prestes a lançar um barril
...
title = pygame.image.load("title-tela.png")
start = pygame.image.load("start.png")
winScreen = pygame.image.load("win-tela.png")
winScreen = pygame.transform.scale(winScreen, (800, 739))
gameOverScreen = pygame.image.load("GameOver.png")
gameOverScreen = pygame.transform.scale(gameOverScreen,(800, 739))
vida = pygame.image.load("vida.png")

level = pygame.image.load("level.png")

pinguimLeft = pygame.image.load("pinguim-left.png")
pinguimRight = pygame.image.load("pinguim-right.png")
runLeft = pygame.image.load("pinguim-runleft.png")
runRight = pygame.image.load("pinguim runRight.png")
pinguimPuloLeft = pygame.image.load("pinguim-left.png")
pinguimPuloRight = pygame.image.load("pinguim-Right.png")
pinguimClimb1 = pygame.image.load("pinguim-Climb1.png")
pinguimImage = pinguimRight

ursoForward = pygame.image.load("polar1.png")
ursoForward = pygame.transform.scale(ursoForward, (125*2, 92*2))
ursoLeft = pygame.image.load("polar1.png")
ursoLeft = pygame.transform.scale(ursoLeft, (125*2, 92*2))
ursoRight = pygame.image.load("polar2.png")
ursoRight = pygame.transform.scale(ursoRight, (125*2, 92*2))
ursoImage = ursoForward

barrilStack = pygame.image.load("barril-stack.png")
barrilDown = pygame.image.load("barril-baixo.png")
barril1 = pygame.image.load("barril1.png")
barril2 = pygame.image.load("barril2.png")
barril3 = pygame.image.load("barril3.png")
barril4 = pygame.image.load("barril4.png")
...
walk = pygame.mixer.Sound("walking\\walking.wav")  # Som do personagem caminhando
pulo = pygame.mixer.Sound("jump\\jump.wav")        # Som do personagem pulando
intro = pygame.mixer.Sound("intro1\\intro1.wav")   # Som da introdução do jogo
death = pygame.mixer.Sound("death\\death.wav")     # Som da morte do personagem
bac = pygame.mixer.music.load("bacmusic\\bacmusic.wav")  # Carrega a música de fundo
...
while replay:
    ...
    while inPlay:
        ...
        # Se urso terminou de subir, mas a introdução ainda não está concluída, passe para o pulo de urso
        if climbDone and introDone == False:
            # Se platNum é menor ou igual a 6, urso ainda está pulando
            if platNum <= 6:
                # Se ursoJumpY é 152, redefine a direção do pulo para começar a descer
                if ursoPuloY == 152:
                    ursoPuloYnum = 10
                
                # Se ursoJumpY é 172, redefine a direção do pulo para começar a subir
                if ursoPuloY == 172:
                    ursoPuloYnum = -10
                    
                    # Altera platNum para mudar a imagem de fundo para que uma plataforma "caia"
                    platNum = platNum + 1
                
                # Move urso para a esquerda 12 pixels
                ursoPuloX = ursoPuloX - 12
                
                # Se platNum não é 6, continue pulando/mudando as coordenadas y
                if platNum != 6:
                    ursoPuloY = ursoPuloY + ursoPuloYnum
        ...
        if gameStart:
            ...
            if dano == False:
                ...
                if not scoreWin:
                    ...
                    #Se throwBarrel for falso, obtenha um número aleatório para decidir se o urso lançará outro barril ou não        
                    if throwBarril == False:
                        #Após cada nível, o intervalo será menor, o que significa uma maior chance de lançar barris
                        ursoChoice = random.randint(0, 50-difficulty)
                    ...
                    #Se throwBarrel for verdadeiro, faça essas alterações
                    if throwBarril:
                        
                        #Adiciona para dar o urso algum tempo para pegar o barril
                        throwCountdown = throwCountdown + 1
                        
                        #Se throwCountdown for 20, crie um novo barril
                        ...
                        #Se throwCountdown atingir 40, redefina as variáveis para quando o urso não estava lançando barril
            ...
            #se nao, pinguim sofre dano, inicia as sequências de morte
                ...
                if morte == False:
                    ...
                    #uando a contagem atinge 60, o curto atraso acabou, redefine variáveis e perde uma vida
                    if deathCount == 60:
                        morte = True
                        deathCount = 0 
                        vidas -= 1
        ...
        if (gameStart and puloEsquerda == False and puloDireita == False and puloParado == False and winLevel == False and dano == False) or gameDone or winGame:      
            ...
            if keys[pygame.K_LEFT] and moveSides and (pinguimX != 320 or pinguimY > 232) and moveLeft and pinguimX != 60:
                ...
                # if pinguium is already facing left, subtract 5 from pinguimX
                if direction == "left":
                    pinguimX = pinguimX - 5
